...
def create_adjacency_matrix_geo(coordinates):
    ...
    for i in range(num_nodes):
        for j in range(i + 1, num_nodes):
            distance = geo_distance(coordinates[i + 1], coordinates[j + 1])  # Using geo_distance function
            adjacency_matrix[i][j] = distance
            adjacency_matrix[j][i] = distance
...
def load_graph_data(filepath):
    ...
    return create_adjacency_matrix(coordinates)
...
def main():
    adjacency_matrix = load_graph_data_geo('data/gr666.tsp')
